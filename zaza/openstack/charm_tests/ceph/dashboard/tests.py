...
class CephDashboardTest(test_utils.BaseCharmTest):
    ...
    def test_saml(self):
        ...
        saml = s.get(location, verify=False, allow_redirects=True).content.decode('utf-8')
        ...
        relay_state = re.search('name="RelayState" value="(.+?)"', content).group(1)
        saml_response = re.search('name="SAMLResponse" value="(.+?)"', content).group(1)
